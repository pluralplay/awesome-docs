...
services:
  ...
  grafana:
    ...
    environment:
      - GF_SERVER_DOMAIN=example.com # основной домен до графаны, не забываем добавить DNS-запись для него
...
server {
    listen 443 ssl;
    server_name example.com; #Вписываем домен для доступа к grafana извне (нужна соотвествующая DNS-запись)
